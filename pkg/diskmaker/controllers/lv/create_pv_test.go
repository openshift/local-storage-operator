...
// TestSyncPVAndLVDL_DeviceLinkArgOrder verifies that SyncPVAndLVDL passes KName
...
// ApplyStatus call inside SyncPVAndLVDL.
//
...
func TestSyncPVAndLVDL_DeviceLinkArgOrder(t *testing.T) {
	reclaimPolicyDelete := corev1.PersistentVolumeReclaimDelete
	kName := "sda"
	...
	// Fetch the LocalVolumeDeviceLink that SyncPVAndLVDL must have created.
	...
	assert.NoError(t, err, "LVDL should have been created by SyncPVAndLVDL")
...
func TestSyncPVAndLVDL_DeviceLinkLifecycle(t *testing.T) {
	reclaimPolicyDelete := corev1.PersistentVolumeReclaimDelete
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// SyncPVAndLVDL, which the stub returns as fakeByIDLink.
