...
func (r *ReconcileLocalVolumeSet) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	localmetrics.SetLVSUnmatchedDevices(nodeName, storageClassName, len(blockDevices)-len(validDevices))
...
func getOrphanedSymlinks(symlinkDir string, validDevices []internal.BlockDevice) ([]string, error) {
